...
namespace Baxter.Vector.Machine
{
    ...
    internal class SolverNu : Solver
    {
        ...
        public override void Solve(int l, QMatrix Q, double[] p, byte[] y,
        ...
        public void SolveIt(int l, QMatrix Q, double[] p, byte[] y,
            double[] alpha, double Cp, double Cn, double eps,
            SolutionInfo si, int shrinking)
        {
            ...
            for (int j = 0; j < active_size; j++)
            {
                ...
                else
                {
                    if (!is_upper_bound(j))
                    {
                        ...
                        {
                            double obj_diff;
                            double quad_coef = QD[idxN] + QD[j] - 2 * Q_in[j];
                            if (quad_coef > 0)
                                obj_diff = -(grad_diff * grad_diff) / quad_coef;
                            else
                                obj_diff = -(grad_diff * grad_diff) / 1e-12;
